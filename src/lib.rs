...
// /// constant).
// ///
// /// These declarations must happen before you declare your
// [`macro@php_startup`] /// function (or [`macro@php_module`] function if you
// do not have a startup /// function).
// ///
// /// # Example
// ///
// /// ```
...
// /// implements [`IntoZval`], and the return type can be anything that
// implements /// [`From<Zval>`] (notice how [`Zval`] is consumed rather than
...
// ///
// /// The last point can be important when interacting with functions that
// return /// unions, such as [`strpos`] which can return an integer or a
// boolean. In this /// case, a [`Zval`] should be returned as parsing a boolean
// to an integer is /// invalid, and vice versa.
// ///
// /// # Example
// ///
// /// This `extern` block imports the [`strpos`] function from PHP. Notice that
// /// the string parameters can take either [`String`] or [`&str`], the
// optional /// parameter `offset` is an [`Option<i64>`], and the return value
// is a [`Zval`] /// as the return type is an integer-boolean union.
// ///
// /// ```
// /// # #![cfg_attr(windows, feature(abi_vectorcall))]
// /// # use ext_php_rs::prelude::*;
...
// ///
// /// Only types that implement [`FromZval`] can be used as parameter and
// return /// types. These include but are not limited to the following:
// ///
...
// /// - [`bool`]
// /// - [`String`]
// /// - [`Vec<T>`] and [`HashMap<String, T>`](std::collections::HashMap) where
// `T: ///   FromZval`.
...
// ///   values.
// /// - [`Option<T>`] where `T: FromZval`. When used as a parameter, the
// parameter ///   will be
// /// deemed nullable, and will contain [`None`] when `null` is passed. When
// used /// as a return type, if [`None`] is returned the [`Zval`] will be set
// to null. /// Optional parameters *must* be of the type [`Option<T>`].
// ///
// /// Additionally, you are able to return a variant of [`Result<T, E>`]. `T`
// must /// implement [`IntoZval`] and `E` must implement `Into<PhpException>`.
// If an /// error variant is returned, a PHP exception is thrown using the
...
// on /// values that you want to be able to be returned from PHP functions.
// ///
...
// ///
// /// Behind the scenes, an `extern "C"` wrapper function is generated, which
// is /// actually called by PHP. The first example function would be converted
// into a /// function which looks like so:
// ///
// /// ```no_run
// /// # #![cfg_attr(windows, feature(abi_vectorcall))]
// /// # use ext_php_rs::{prelude::*, exception::PhpException,
// zend::ExecuteData, convert::{FromZvalMut, IntoZval}, types::Zval, args::{Arg,
// ArgParser}}; /// pub fn hello(name: String) -> String {
// ///     format!("Hello, {}!", name)
// /// }
// ///
// /// pub extern "C" fn _internal_php_hello(ex: &mut ExecuteData, retval: &mut
// Zval) { ///     let mut name = Arg::new("name", <String as
// FromZvalMut>::TYPE); ///     let parser = ex.parser()
// ///         .arg(&mut name)
// ///         .parse();
// ///
// ///     if parser.is_err() {
...
// ///         Some(val) => val,
// ///         None => {
// ///             PhpException::default("Invalid value given for argument
// `name`.".into()) ///                 .throw()
// ///                 .expect("Failed to throw exception: Invalid value given
// for argument `name`."); ///             return;
// ///         }
// ///     });
// ///
...
// ///             e.throw().expect("Failed to throw exception: Failed to set
// return value."); ///         }
// ///     };
// /// }
// /// ```
...
// /// ```
// ///
// /// Parameters can also be deemed optional by passing the parameter name in
// the /// attribute options. This function takes one required parameter
// (`name`) and /// two optional parameters (`description` and `age`).
// ///
// /// ```
// /// # #![cfg_attr(windows, feature(abi_vectorcall))]
// /// # use ext_php_rs::prelude::*;
// /// #[php_function(optional = "description")]
// /// pub fn hello(name: String, description: Option<String>, age: Option<i32>)
// -> String { ///     let mut response = format!("Hello, {}!", name);
// ///
...
// ///
// ///     if let Some(age) = age {
// ///         response.push_str(format!(" I am {} year(s) old.",
// age).as_ref()); ///     }
// ///
// ///     response
// /// }
...
// /// function could have default values for `description` and `age` by
// changing /// the attribute to the following:
// ///
// /// ```
// /// # #![cfg_attr(windows, feature(abi_vectorcall))]
// /// # use ext_php_rs::prelude::*;
// /// #[php_function(optional = "description", defaults(description = "David",
// age = 10))] /// pub fn hello(name: String, description: String, age: i32) ->
// String { ///     format!("Hello, {}! {}. I am {} year(s) old.", name,
// description, age) /// }
// /// # #[php_module]
// /// # pub fn module(module: ModuleBuilder) -> ModuleBuilder {
// /// #     module
...
// /// Annotates a structs `impl` block, declaring that all methods and
// constants /// declared inside the `impl` block will be declared as PHP
// methods and /// constants.
// ///
// /// If you do not want to export a method to PHP, declare it in another
// `impl` /// block that is not tagged with this macro.
// ///
// /// The declared methods and functions are kept intact so they can continue
// to /// be called from Rust. Methods do generate an additional function, with
// an /// identifier in the format `_internal_php_#ident`.
// ///
...
// ///
// /// The main difference is that the contents of the `impl` block *do not*
// need /// to be tagged with additional attributes - this macro assumes that
// all /// contents of the `impl` block are to be exported to PHP.
// ///
// /// The only contrary to this is setting the visibility, optional argument
// and /// default arguments for methods. These are done through separate
// macros: ///
// /// - `#[defaults(key = value, ...)]` for setting defaults of method
// variables, ///   similar to the
...
// /// arguments).
// /// - `#[public]`, `#[protected]` and `#[private]` for setting the visibility
// of ///   the method,
...
// /// consume `self`. They can also take no reference to `self` which indicates
// a /// static method.
// ///
// /// ## Constructors
// ///
// /// You may add *one* constructor to the impl block. This method must be
// called /// `__construct` or be tagged with the `#[constructor]` attribute,
// and it will /// not be exported to PHP like a regular method.
// ///
// /// The constructor method must not take a reference to `self` and must
// return /// `Self` or [`Result<Self, E>`][`Result`], where `E:
// Into<PhpException>`. ///
// /// # Example
// ///
// /// ```no_run
...
// /// Annotates a function that will be used by PHP to retrieve information
// about /// the module.
// ///
// /// In the process, the function is wrapped by an `extern "C"` function which
// is /// called from PHP, which then calls the given function.
// ///
// /// As well as wrapping the function, the `ModuleBuilder` is initialized and
// /// functions which have already been declared with the
// [`macro@php_function`] /// attribute will be registered with the module, so
// ideally you won't have to /// do anything inside the function.
// ///
// /// The attribute must be called on a function *last*, i.e. the last
// proc-macro /// to be compiled, as the attribute relies on all other PHP
// attributes being /// compiled before the module. If another PHP attribute is
// compiled after the /// module attribute, an error will be thrown.
// ///
// /// Note that if the function is not called `get_module`, it will be renamed.
// ///
// /// If you have defined classes using the [`macro@php_class`] macro and you
// have /// not defined a startup function, it will be automatically declared
// and /// registered.
// ///
// /// # Example
// ///
// /// The `get_module` function is required in every PHP extension. This is a
// bare /// minimum example, since the function is declared above the module it
// will /// automatically be registered when the module attribute is called.
// ///
// /// ```
// /// # #![cfg_attr(windows, feature(abi_vectorcall))]
// /// # use ext_php_rs::prelude::*;
...
// ///
// /// * `name` - The name of the exported class, if it is different from the
// Rust ///   struct name. This can be useful for namespaced classes, as you
// cannot ///   place backslashes in Rust struct names.
// ///
...
// /// startup function (which is annotated by the [`macro@php_startup`] macro,
// or /// automatically generated just above the [`macro@php_module`] function).
// ///
// /// Fields defined on the struct *are not* the same as PHP properties, and
// are /// only accessible from Rust.
// ///
// /// # Example
// ///
...
// /// Annotates a function that will be called by PHP when the module starts
// up. /// Generally used to register classes and constants.
// ///
// /// As well as annotating the function, any classes and constants that had
// been /// declared using the [`macro@php_class`], [`macro@php_const`] and
// /// [`macro@php_impl`] attributes will be registered inside this function.
// ///
// /// This function *must* be declared before the [`macro@php_module`]
// function, /// as this function needs to be declared when building the module.
// ///
// /// This function will automatically be generated if not already declared
// with /// this macro if you have registered any classes or constants when
// using the /// [`macro@php_module`] macro.
// ///
// /// # Example
// ///
// /// ```
...
// /// [`Zval`]. Both [`FromZval`] and [`IntoZval`] are implemented on types
// which /// use this macro.
// ///
// /// # Structs
// ///
// /// When the macro is used on a struct, the [`FromZendObject`] and
// /// [`IntoZendObject`] traits are also implemented, and will attempt to
// retrieve /// values for the struct fields from the objects properties. This
// can be useful /// when you expect some arbitrary object (of which the type
// does not matter), /// but you care about the value of the properties.
// ///
// /// All properties must implement [`FromZval`] and [`IntoZval`] themselves.
// /// Generics are supported, however, a [`FromZval`] and [`IntoZval`] bound
// will /// be added. If one property cannot be retrieved from the object, the
...
// /// implementations will treat the enum as a tagged union with a mixed
// datatype. /// This allows you to accept two different types in a parameter,
...
// ///
// /// The ordering of the enum variants is important, as the [`Zval`] contents
// is /// matched in order of the variants. For example, [`Zval::string`] will
// attempt /// to read a string from the [`Zval`], and if the [`Zval`] contains
// a long, the /// long will be converted to a string. If a string variant was
// placed above an /// integer variant in the enum, the integer would be
// converted into a /// string and passed as the string variant.
// ///
// /// ## Examples
// ///
// /// Basic example showing the importance of variant ordering and default
// field: ///
// /// ```
// /// # #![cfg_attr(windows, feature(abi_vectorcall))]
// /// # use ext_php_rs::prelude::*;
