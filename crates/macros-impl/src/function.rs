...
pub fn parser(args: AttributeArgs, input: ItemFn) -> Result<(TokenStream, Function)> {
    ...
    let func = quote! {
        ...
        ::awl::ms::php::zend_fastcall! {
            #[doc(hidden)]
            pub extern fn #internal_ident(ex: &mut ::awl::ms::php::zend::ExecuteData, retval: &mut ::awl::ms::php::types::Zval) {
        ...
        ParserType::Method => (
            ...
            Some(quote! {
                let this = match this {
                    Some(this) => this,
                    None => {
                        ::awl::ms::php::exception::PhpException::default("Failed to retrieve reference to `$this`".into())
                            .throw()
                            .unwrap();
                        return;
                    },
...
impl Arg {
    ...
    pub fn get_type_ident(&self) -> TokenStream {
        ...
        quote! {
            <#ty as ::awl::ms::php::convert::FromZvalMut>::TYPE
        }
    }
        ...
        if let Some(default) = self.default.as_ref() {
            ...
            quote! {
                match #name_ident.val() {
                    Some(val) => val,
                    None => {
                        ::awl::ms::php::exception::PhpException::default(
    ...
    pub fn get_arg_definition(&self) -> TokenStream {
        ...
        quote! {
            ::awl::ms::php::args::Arg::new(#name, #ty) #null #default
        }
    }
}
...
impl Function {
    ...
    pub fn get_builder(&self) -> TokenStream {
        ...
        quote! {
            ::awl::ms::php::builders::FunctionBuilder::new(#name, #name_ident)
                #(#args)*
                #output
                .build()
        }
